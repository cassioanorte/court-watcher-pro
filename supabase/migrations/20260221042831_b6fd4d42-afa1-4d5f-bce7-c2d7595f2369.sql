...
-- Add manual override columns to staff_case_access
ALTER TABLE public.staff_case_access
ADD COLUMN IF NOT EXISTS blocked_case_ids uuid[] DEFAULT '{}',
ADD COLUMN IF NOT EXISTS extra_case_ids uuid[] DEFAULT '{}';

-- Update can_view_case to respect manual overrides
...
AS $function$
  SELECT
    ...
    (
      ...
      AND (
        -- Check manual overrides first: explicitly blocked = deny
        NOT EXISTS (
      ...
      AND (
        -- Explicitly allowed via extra_case_ids = grant
        EXISTS (
          SELECT 1 FROM staff_case_access sca
          WHERE sca.user_id = _user_id
          AND sca.tenant_id = get_user_tenant_id(_user_id)
        ...
        OR
        -- No config row: default to own cases (by responsible_user_id OR by OAB in publications)
        (
