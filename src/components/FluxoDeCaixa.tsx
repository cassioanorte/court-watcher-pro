...
const FluxoDeCaixa = () => {
  ...
  const netCash = totalRevenue - totalExpense - totalDistributed;

  // Sacado orders with their transactions
  const sacadoOrders = orders.filter(o => o.status === "sacado");
  const pendingOrders = orders.filter(o => o.status !== "sacado" && o.status !== "cancelado");

  // Timeline: group by RPV sacado
  const rpvTimeline = useMemo(() => {
    return sacadoOrders.map(order => {
      ...
      return {
        ...
        officeGross,
        ir,
        netOffice: Number(order.office_amount),
        distributions: orderDistributions,
        totalDistributed: totalDist,
        availableForRateio: Math.max(availableForRateio, 0),
      };
    });
  ...
  const monthlySummary = useMemo(() => {
    const months: Record<string, { revenue: number; expense: number; distributed: number }> = {};
    filteredTx.forEach(t => {
      const key = format(parseISO(t.date), "yyyy-MM");
      if (!months[key]) months[key] = { revenue: 0, expense: 0, distributed: 0 };
    ...
    return Object.entries(months)
      ...
      .map(([month, data]) => ({
        ...
        ...data,
        net: data.revenue - data.expense - data.distributed,
      }));
  }, [filteredTx, distributions]);

  // Expense breakdown (general office expenses vs process-related)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Saídas (Despesas)", value: totalExpense, icon: ArrowDownRight, color: "text-red-500", bg: "bg-red-500/10" },
                  ...
                  <div className="border-t px-4 pb-4 pt-3 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 text-center text-xs">
                      ...
                      <div className="bg-red-500/10 rounded-lg p-2">
                        ...
                        <p className="font-bold text-red-500 text-sm">−{fmt(item.ir)}</p>
                      </div>
                      <div className="flex items-center justify-center text-muted-foreground font-bold">→</div>
