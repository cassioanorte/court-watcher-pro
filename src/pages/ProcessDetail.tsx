...
const ProcessDetail = () => {
  ...
  const openEditModal = async () => {
    ...
    setEditForm({
      ...
      responsible_user_ids: (caseData as any).responsible_user_ids?.length ? (caseData as any).responsible_user_ids : (caseData.responsible_user_id ? [caseData.responsible_user_id] : []),
        ...
        <div className="bg-card rounded-lg p-4 border shadow-card col-span-1 md:col-span-2 lg:col-span-1">
          <div className="flex items-center justify-between mb-2">
            ...
            {isLawyer && !editingNextStep && (
              <button onClick={() => setEditingNextStep(true)} className="text-[10px] text-accent hover:underline flex items-center gap-1">
                <Plus className="w-3 h-3" /> Nova anotação
              </button>
            )}
          </div>
          ...
          {editingNextStep && isLawyer && (
            <div className="flex gap-2 mb-3">
              <input
                ...
                className="flex-1 h-9 px-3 rounded-lg bg-background border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/40"
                placeholder="Escreva uma anotação..."
              />
              <button onClick={handleAddNote} disabled={addingNote || !newNoteText.trim()} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg gradient-accent text-accent-foreground text-xs font-semibold disabled:opacity-50">
                {addingNote ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3" />} Adicionar
              </button>
              <button onClick={() => { setEditingNextStep(false); setNewNoteText(""); }} className="px-3 py-1.5 rounded-lg border text-xs text-muted-foreground hover:text-foreground">Cancelar</button>
            </div>
          )}
          ...
          {caseNotes.length > 0 ? (
            <div className="space-y-1.5 max-h-48 overflow-y-auto">
              {caseNotes.map((note) => (
                <div key={note.id} className="flex items-center gap-2 group">
                  <button
                    onClick={() => handleToggleNote(note.id, !note.completed)}
                    className={cn(
                      "w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors",
                      note.completed ? "bg-accent border-accent" : "border-border hover:border-accent"
                    )}
                  >
                    {note.completed && <span className="text-accent-foreground text-[10px]">✓</span>}
                  </button>
                  <p className={cn("text-sm flex-1", note.completed ? "line-through text-muted-foreground" : "text-foreground")}>
                    {note.text}
                  </p>
                  {isLawyer && (
                    <button
                      ...
                      className="p-1 rounded text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors shrink-0 opacity-0 group-hover:opacity-100"
            ...
            !editingNextStep && <p className="text-sm text-muted-foreground">Nenhuma anotação</p>
          )}
        </div>
      </div>
            ...
            <form onSubmit={handleSaveFullEdit} className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-1 space-y-1 max-h-40 overflow-y-auto rounded-lg border bg-background p-2">
                  {editStaff.length === 0 && <p className="text-xs text-muted-foreground">Nenhum advogado disponível</p>}
                  {editStaff.map(s => (
