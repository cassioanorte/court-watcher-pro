...
const ClassicTemplate = ({ content }: { content: LPContent }) => (
  <div className="min-h-screen bg-white text-gray-900">
      ...
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="flex flex-wrap justify-center gap-8">
          {content.contactPhone && (
            <a href={`tel:${content.contactPhone}`} className="flex items-center gap-2 text-slate-300 hover:text-amber-400 transition-colors">
              <Phone className="w-5 h-5" /> {content.contactPhone}
            </a>
          )}
          {content.contactEmail && (
            <a href={`mailto:${content.contactEmail}`} className="flex items-center gap-2 text-slate-300 hover:text-amber-400 transition-colors">
              <Mail className="w-5 h-5" /> {content.contactEmail}
            </a>
          )}
          {content.contactAddress && (
            <span className="flex items-center gap-2 text-slate-300">
              <MapPin className="w-5 h-5" /> {content.contactAddress}
            </span>
          )}
        </div>
      </div>
    ...
    {/* Footer */}
    <footer className="py-6 px-6 bg-slate-950 text-center text-sm text-slate-500">
      {content.footerText}
    </footer>
  </div>
);

const ModernTemplate = ({ content }: { content: LPContent }) => (
  <div className="min-h-screen bg-white text-gray-900">
    {/* Hero */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {content.services.map((svc, i) => (
              <div key={i} className="bg-white p-8 rounded-2xl shadow-md border hover:shadow-lg transition-shadow hover:-translate-y-1">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-lg mb-4">
                  {i + 1}
                </div>
    ...
    <section id="contato" className="py-20 px-6 bg-gradient-to-br from-indigo-600 to-purple-600 text-white">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="flex flex-wrap justify-center gap-8">
          {content.contactPhone && (
            <a href={`tel:${content.contactPhone}`} className="flex items-center gap-2 text-white/80 hover:text-white transition-colors">
              <Phone className="w-5 h-5" /> {content.contactPhone}
            </a>
          )}
          {content.contactEmail && (
            <a href={`mailto:${content.contactEmail}`} className="flex items-center gap-2 text-white/80 hover:text-white transition-colors">
              <Mail className="w-5 h-5" /> {content.contactEmail}
            </a>
          )}
          {content.contactAddress && (
...
const MinimalTemplate = ({ content }: { content: LPContent }) => (
  <div className="min-h-screen bg-white text-gray-900">
    {/* Hero */}
    ...
    <section id="contato" className="py-20 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-3">
          {content.contactPhone && (
            <a href={`tel:${content.contactPhone}`} className="flex items-center gap-3 text-gray-600 hover:text-gray-900 transition-colors">
              <Phone className="w-4 h-4" /> {content.contactPhone}
            </a>
          )}
          {content.contactEmail && (
            <a href={`mailto:${content.contactEmail}`} className="flex items-center gap-3 text-gray-600 hover:text-gray-900 transition-colors">
              <Mail className="w-4 h-4" /> {content.contactEmail}
            </a>
          )}
          {content.contactAddress && (
            <span className="flex items-center gap-3 text-gray-600">
              <MapPin className="w-4 h-4" /> {content.contactAddress}
            </span>
          )}
        </div>
      </div>
    ...
    <footer className="py-6 px-6 border-t text-center text-xs text-gray-400">
      {content.footerText}
    </footer>
  </div>
);

const TEMPLATE_MAP: Record<string, React.FC<{ content: LPContent }>> = {
